...
router.get('/stats', (req, res) => {
  try {
    ...
    const identified = db.prepare("SELECT COUNT(*) as count FROM alumni WHERE status = 'Teridentifikasi'").get();
    const needsVerification = db.prepare("SELECT COUNT(*) as count FROM alumni WHERE status = 'Perlu Verifikasi Manual'").get();
    const notFound = db.prepare("SELECT COUNT(*) as count FROM alumni WHERE status = 'Belum Ditemukan'").get();
...
router.get('/:id', (req, res) => {
  try {
    const alumni = db.prepare('SELECT * FROM alumni WHERE id = ?').get(req.params.id);
...
router.post('/', (req, res) => {
  try {
    ...
    if (!nama || !tahun_lulus || !program_studi) {
      return res.status(400).json({ error: 'Nama, tahun lulus, dan program studi wajib diisi' });
    }
    ...
    const alumni = db.prepare('SELECT * FROM alumni WHERE id = ?').get(result.lastInsertRowid);
...
router.put('/:id', (req, res) => {
  try {
    ...
    const existing = db.prepare('SELECT * FROM alumni WHERE id = ?').get(req.params.id);
    if (!existing) return res.status(404).json({ error: 'Alumni tidak ditemukan' });
